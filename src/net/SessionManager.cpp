...
std::shared_ptr<Session> SessionManager::CreateSession(std::shared_ptr<GameWorker> in_game_worker)
{
    std::lock_guard<std::mutex> lock(m_mutex);

    const SessionId session_id = m_next_session_id++;
    auto session = std::make_shared<Session>(m_io, session_id, in_game_worker);
