...
bool PacketHandler::Dispatch(const PacketContext& in_ctx)
{
    ...
    {
        LOG_WARN("packet too small from session " + std::to_string(in_ctx.TargetSessionId));
        return false;
    }
    ...
    switch (static_cast<PacketId>(opcode))
    {
        ...
        default:
            LOG_WARN("unknown opcode " + std::to_string(opcode)
                     + " from session " + std::to_string(in_ctx.TargetSessionId));
            return false;
    }
...
void PacketHandler::HandleLogin(const PacketContext& in_ctx)
{
    ...
    {
        LOG_WARN("LOGIN_REQ payload too small from session " + std::to_string(in_ctx.TargetSessionId));
        return;
    }
    ...
    LOG_INFO("LOGIN_REQ session=" + std::to_string(in_ctx.TargetSessionId)
    ...
    auto user = m_user_manager->CreateUser(account_id, in_ctx.TargetSessionId);
    if (!user)
    {
        ...
        in_ctx.TargetSession->SendPacket(response.Build());
        return;
    }

    in_ctx.TargetSession->BindUser(user);
    ...
    if (m_db_worker)
    {
        Job db_job(JobType::DB_LOG_LOGIN, in_ctx.TargetSessionId, in_ctx.PacketData);
...
{
    LOG_DEBUG("ECHO_REQ from session " + std::to_string(in_ctx.TargetSessionId));
...
{
    LOG_DEBUG("HEARTBEAT_REQ from session " + std::to_string(in_ctx.TargetSessionId));
...
void PacketHandler::HandleMove(const PacketContext& ctx) 
{
    ...
    auto self = ctx.TargetSession->GetUser();
    ...
    ack.Write(nearby);
    ctx.TargetSession->SendPacket(ack.Build());
}
